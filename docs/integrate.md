...
- **Server requires a provider key** at runtime (`OPENAI_API_KEY`, `GOOGLE_API_KEY`, `MISTRAL_API_KEY`, `DEEPGRAM_API_KEY`, or `AWS_BEARER_TOKEN_BEDROCK`) for TTS, transcription, and judge calls. Deepgram covers transcription (Nova-3 — word-level timings + language detection) and TTS (Aura-2 — language-specific voices incl. German/French/Spanish/Italian/Dutch/Japanese) but not the judge; AWS Bedrock covers the judge only (Claude Opus 4.8 by default) — pair each with another key for the remaining stages. Bedrock prerequisites live in your AWS account: the key's identity needs `bedrock:CallWithBearerToken` + invoke permissions, and Anthropic models additionally need model access enabled (an AWS Marketplace subscription — one-time, in the Bedrock console). You can override each stage: `XRAY_TTS_PROVIDER`, `XRAY_TRANSCRIPTION_PROVIDER`, and `XRAY_JUDGE_PROVIDER`, plus `XRAY_TTS_MODEL`, `XRAY_TTS_VOICE`, `XRAY_TRANSCRIPTION_MODEL`, `XRAY_JUDGE_MODEL`, and `XRAY_BEDROCK_REGION` (bedrock-runtime endpoint region, default `us-east-1`).
