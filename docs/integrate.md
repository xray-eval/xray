...
- **Three-stage server chain.** `/analyze` now enqueues `analyze-replay` (VAD plus per-turn transcription through the configured STT provider; each turn's slice covers its full attribution window, not just the VAD-detected speech). That enqueues `calculate-metrics` (agent_response_ms, interrupted). That enqueues `evaluate-replay`, which runs all assertions and judges, then emits the `evaluation_complete` SSE. (VAD means voice activity detection: finding which parts of the audio contain speech.)
