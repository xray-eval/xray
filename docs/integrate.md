...
- `TtsAudio()` (or no `audio` at all). This is synthesized **server-side** during the conversation upsert. (TTS means text-to-speech: turning written text into spoken audio.) The xray server uses whichever provider it is configured with (`XRAY_TTS_PROVIDER`: OpenAI, Google, Mistral, or Deepgram). Pass `TtsAudio(language="de")` for non-English turns — Mistral voices are language-specific, and the server then picks a voice matching the language (from its presets or your org's cloned voices) instead of an English voice speaking the text with an accent. The generated audio is stored content-addressed, and its sha256 is part of the conversation hash. The driver pulls the exact bytes back before joining the room. There is no TTS key in the SDK's process.

For Cartesia or 11Labs, synthesize the audio yourself and pass the output as `RecordedAudio`. Adding more server-side TTS providers is one file each in `src/server/tts/`.
...
- **Three-stage server chain.** `/analyze` now enqueues `analyze-replay` (VAD plus per-turn transcription through the configured STT provider; each turn's slice covers its full attribution window, not just the VAD-detected speech, with trailing silence trimmed off so a recording that outlives the conversation doesn't hand the provider a minute of nothing). That enqueues `calculate-metrics` (agent_response_ms, interrupted, yield_ms). That enqueues `evaluate-replay`, which runs all assertions and judges, then emits the `evaluation_complete` SSE. (VAD means voice activity detection: finding which parts of the audio contain speech.)
...
- **Server requires a provider key** at runtime (`OPENAI_API_KEY`, `GOOGLE_API_KEY`, `MISTRAL_API_KEY`, `DEEPGRAM_API_KEY`, or `AWS_BEARER_TOKEN_BEDROCK`) for TTS, transcription, and judge calls. Deepgram covers transcription (Nova-3 — word-level timings + language detection) and TTS (Aura-2 — language-specific voices incl. German/French/Spanish/Italian/Dutch/Japanese) but not the judge; AWS Bedrock covers the judge only (Claude Opus 4.8 by default) — pair each with another key for the remaining stages. Bedrock auth accepts either form: a Bedrock API key via `AWS_BEARER_TOKEN_BEDROCK`, or standard SigV4 access keys via `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY` (+ `AWS_SESSION_TOKEN` for temporary/assumed-role credentials) — signed in-process, no AWS SDK. Bearer wins if both are set. The SigV4 vars are used **only** when `XRAY_JUDGE_PROVIDER=bedrock` is set explicitly, so ambient AWS credentials never auto-select the judge. Account prerequisites: the identity needs Bedrock invoke permissions, and Anthropic models additionally need model access enabled (an AWS Marketplace subscription — one-time, per region, in the Bedrock console; enablement is regional, so set `XRAY_BEDROCK_REGION` to a region where Opus access is granted and pair it with the matching geo model id, e.g. `XRAY_JUDGE_MODEL=eu.anthropic.claude-opus-4-8`). You can override each stage: `XRAY_TTS_PROVIDER`, `XRAY_TRANSCRIPTION_PROVIDER`, and `XRAY_JUDGE_PROVIDER`, plus `XRAY_TTS_MODEL`, `XRAY_TTS_VOICE`, `XRAY_TRANSCRIPTION_MODEL`, `XRAY_JUDGE_MODEL`, and `XRAY_BEDROCK_REGION` (bedrock-runtime endpoint region, default `us-east-1`).
