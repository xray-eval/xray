...
/**
 * Walk a request turn array, hash each referenced audio part, synthesize
 * each `{kind: "tts"}` turn via `synthesizeTurn`, and produce the
 * canonical/stored form: `{kind: "recorded", sha256}` substituted in for
 * `{kind: "recorded", upload_key}`, `{kind: "tts", sha256, voice_id?}`
 * substituted in for `{kind: "tts", voice_id?}`. The synthesized sha256
 * makes the *generated bytes* part of the conversation hash — same
 * identity rule as recorded audio.
 *
 ...
 * audio. Throws `TtsTurnMissingTextError` / `TtsTurnRoleError` for tts
 * refs that can't be synthesized.
 */
