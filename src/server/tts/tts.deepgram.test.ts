...
describe("createDeepgramTtsProvider", () => {
	...
	it("wraps an AbortSignal timeout on synthesize with a timeout-specific message", async () => {
		...
		const provider = createDeepgramTtsProvider({ apiKey: () => "dg", fetchImpl, timeoutMs: 5_000 });
		const err = await provider.synthesize({ text: "x", voice: "v" }).then(
			() => null,
			(e: unknown) => e,
		);
	...
	it("wraps a caller-triggered abort on synthesize with an abort-specific message", async () => {
		...
		const err = await provider
			.synthesize({ text: "x", voice: "v", signal: controller.signal })
			.then(
				() => null,
				(e: unknown) => e,
			);
	...
	it("wraps a rejected models-catalog fetch as a generic fetch failure", async () => {
		const fetchImpl: FetchLike = async () => {
			throw new Error("simulated network failure");
		};
		const provider = createDeepgramTtsProvider({ apiKey: () => "dg", fetchImpl });
		...
		expect(err.message).toContain("models catalog fetch failed");
	});

	it("falls back to '<unreadable body>' when the models-catalog error body stream errors mid-read", async () => {
		const fetchImpl: FetchLike = async () =>
			new Response(
				new ReadableStream({
					start(controller) {
	...
	it("throws TtsProviderError when the models-catalog body's JSON parse throws", async () => {
		const fetchImpl: FetchLike = async () =>
		...
		expect(err.message).toContain("models catalog body was not valid JSON");
	});
});
