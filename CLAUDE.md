...
1. **Control plane (the SDK calls these directly).**
   ...
   - `GET /v1/run-configs`, `POST /v1/run-configs/compare`, `GET /v1/run-configs/:hash` — read-only config-group comparison: coverage per group, cross-conversation metric aggregates (avg + p50 + p95 + sample size), and the per-conversation drill-down that links each row to its newest included replay (under `all` selection the row's metrics span every replay it lists, not just that one). Percentiles are computed in TS (`run-configs.aggregate.ts` is pure) because SQLite has no percentile function.
