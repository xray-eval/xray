...
### `package.json`
- `"packageManager": "pnpm@11.20.0"` (or newer 11.x) — pinned, activated via `corepack enable`. `Dockerfile.dev.server` mirrors it explicitly (corepack cannot read `packageManager` before the COPY); keep the two equal.
