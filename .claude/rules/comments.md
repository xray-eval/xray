...
|---|---|---|
| **Why this code exists** — a constraint, deadline, incident, or external requirement that the code itself can't express | ✅ | `// Trust boundary: OTLP receiver never creates Conversation or Replay rows — only the SDK's POSTs can. Drop unknown replay_ids silently rather than gating.` |
...
| **Sentinel for codegen / structured tooling** — codegen guards, drizzle-kit markers, etc. (linter suppressions are banned — see [`no-lint-suppressions.md`](./no-lint-suppressions.md)) | ✅ | `// drizzle-kit: do not edit below this line` |
