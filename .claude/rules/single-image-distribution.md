...
- **Never reach for an ORM that abstracts over engines** (Prisma, Drizzle's multi-engine mode) on the grounds of "future flexibility." The future is also SQLite. Use `bun:sqlite` directly or a thin query builder pinned to it.
...
- **Cross-reference this rule** whenever a PR adds a service container to `compose.yaml`, a DB driver to `package.json`, or a `DATABASE_URL` environment variable. Any of those three is the smoke signal.
